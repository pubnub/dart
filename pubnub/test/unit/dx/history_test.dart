...
void main() {
  ...
  group('DX [history]', () {
    ...
    test('.batch#fetchMessages default count for single channel', () async {
      when(
              path:
                  '/v3/history/sub-key/test/channel/my_channel?max=100&include_message_type=true&include_uuid=true&pnsdk=PubNub-Dart%2F${PubNub.version}',
              method: 'GET')
          .then(status: 200, body: _batchFetchMessagesSuccessResponse);
    ...
    test('.batch#fetchMessages default count for multiple channel', () async {
      when(
              path:
                  '/v3/history/sub-key/test/channel/ch1,ch2?max=25&include_message_type=true&include_uuid=true&pnsdk=PubNub-Dart%2F${PubNub.version}',
              method: 'GET')
          .then(status: 200, body: _batchFetchMessagesSuccessResponse);
    ...
    test('.batch#fetchMessages with actions default count', () async {
      when(
              path:
                  '/v3/history-with-actions/sub-key/test/channel/ch1?max=25&include_message_type=true&include_uuid=true&pnsdk=PubNub-Dart%2F${PubNub.version}',
              method: 'GET')
          .then(
    ...
    test('.batch#fetchMessages with include_meta', () async {
      when(
              path:
                  '/v3/history/sub-key/test/channel/my_channel?max=100&include_meta=true&include_message_type=true&include_uuid=true&pnsdk=PubNub-Dart%2F${PubNub.version}',
              method: 'GET')
